...
def createLidarPlot(fig:plt.Figure, gs:gridspec.GridSpec):
    ...
    def draw_rectangle(ax, center_x, center_y, width, height, color='blue'):
        # Get 4 corners in Cartesian
        w = width / 2
        h = height / 2
        corners = np.array([
        ...
        # Draw edges as plain lines
        for i in range(len(corners)):
            start = corners[i]
            end = corners[(i + 1) % len(corners)]

            r1 = np.hypot(*start)
            theta1 = np.arctan2(start[1], start[0])
            r2 = np.hypot(*end)
            theta2 = np.arctan2(end[1], end[0])
    ...
    draw_rectangle(ax, center_x=0, center_y=0, width=300, height=200, color='blue')

    draw_rectangle(ax, center_x=-200, center_y=0, width=100, height=400, color='green')
