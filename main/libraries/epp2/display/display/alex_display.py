...
class LidarBasicDisplay(PubSubDisplayTemplate):
    ...
    def configurePlot(self, ax):
        ...
        ax.set_title("WADDUP BRO", pad=20)
        initial_angle = np.pi - np.array(self.initial_data[0][:self.maximumSamplesinRound])/180 * np.pi
    ...
    def update(self, frame, data, extra):
        ...
        angle = np.pi - (np.array(angle))/180 * np.pi
        dist = np.array(dist)
